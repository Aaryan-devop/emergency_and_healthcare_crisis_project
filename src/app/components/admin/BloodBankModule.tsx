    ...
    <div className="p-6 space-y-4">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Blood Inventory">
        <form onSubmit={handleAddInventory} className="space-y-4">
          <div>
            <label className="text-sm font-medium text-slate-300 block mb-1">Blood Type</label>
            <select value={newBlood.type} onChange={e => setNewBlood({...newBlood, type: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-red-500">
          ...
          <div>
            <label className="text-sm font-medium text-slate-300 block mb-1">Units to Add</label>
            <input type="number" required min="1" value={newBlood.units} onChange={e => setNewBlood({...newBlood, units: parseInt(e.target.value)})} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-red-500" />
          </div>
